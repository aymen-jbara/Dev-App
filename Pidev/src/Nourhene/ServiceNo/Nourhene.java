...
/*import sample.Nourhene.Maps.MapController;
import sample.Nourhene.Maps.WebViewCaptureMap;
import sample.Nourhene.Entity.Moyen;
import sample.Nourhene.Entity.Stat;
import sample.Nourhene.Entity.Vol;*/
...
public class Nourhene <file> implements Initializable {
    ...
    @FXML
    private TableColumn<Vol, Date> coldatea;
   ...
   /* file = new ArrayList<>();
    ...
    public ObservableList<Vol> getVolList() {
        ...
        try {
            ...
            Vol vol;
            while (rs.next()) {
                vol = new Vol(rs.getString("nomv"), rs.getString("numv"), rs.getDate("dated"), rs.getDate("datea"), rs.getString("chauffeur"), rs.getString("depart"), rs.getString("arriver"), rs.getString("prix"));
                volList.add(vol);
            }
    ...
    public void showVol() {
        ...
        coldatea.setCellValueFactory(new PropertyValueFactory<Vol, Date>("datea"));
    ...
    void btn_ajoutv(ActionEvent event) {
        ...
        if (errors.length() > 0) {
            ...
            try {
                String st = "INSERT INTO vol (nomv,numv,dated,datea,chauffeur,depart,arriver,prix)VALUES(?,?,?,?,?,?,?,?)";
                Connection conn = getConnection();
                preparedStatement = (PreparedStatement) conn.prepareStatement(st);
                // preparedStatement.setString(1, random_id());
    ...
    void btn_suppv(ActionEvent event) {
        ...
        String query = "DELETE FROM vol WHERE numv='" + vol.getNumv() + "'";
        executeQuery(query);
        TrayNotification tray = new TrayNotification();
        AnimationType type = AnimationType.POPUP;
        tray.setAnimationType(type);
    ...
    public String btn_updatev(javafx.event.ActionEvent actionEvent) {
        try {
            String st = "UPDATE  vol SET nomv =?,dated =?, datea=? , chauffeur=?, depart=?, arriver=? ,prix=? WHERE numv = ? ";
            Connection conn = getConnection();
            preparedStatement = (PreparedStatement) conn.prepareStatement(st);
            preparedStatement.setString(1, nomv.getText());
    ...
    void JEUX(ActionEvent event) throws IOException{
            if (event.getSource() == GAME1) {
               // GAME1.getScene().getWindow().hide();
                Parent root = FXMLLoader.load(Objects.requireNonNull(getClass().getClassLoader().getResource("Nourhene/jeux/Nourhene.jeux.FXML")));
